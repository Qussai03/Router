...
std::optional<RoutingEntry> RoutingTable::getRoutingEntry(ip_addr ip) {
    ...
    int bestPrefixLen = -1;  // so even a /0 default route (prefixLen = 0) can win

    for (const auto& entry : routingEntries) {
        // IPs and masks are all stored in network order; bitwise match is fine.
        if ((ip & entry.mask) == (entry.dest & entry.mask)) {
            // count number of 1-bits in the mask = prefix length
            int prefixLen = __builtin_popcount(entry.mask);
